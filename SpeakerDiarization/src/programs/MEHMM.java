...
import parameter.ParameterEHMM;
...
public class MEHMM {
	 ...
	 * @throws Exception the exception
	 */
	public static ClusterSet make2Spk(AudioFeatureSet features, ClusterSet clusters, GMM ubm,GMMArrayList spk_2_Vec, Parameter param) throws Exception {
		...
		GMM gmmS0 = speakersList.get(0);//得到第一个speaker的GMM
		...
		int n = 30;
		while (current.equals(previous) == false && (n--)>0)
		{
			previous = current;
		...
		spk_2_Vec.add(speakersList.get(0));
		spk_2_Vec.add(speakersList.get(1));
		
		return current;
	}
	
	/** 2013-12-27为了提出 gmmList 做了更改
	 * Make2 spk.
	 * 
	 * @param features the features
	 * @param clusters the clusters
